...
export default async function CashflowReport() {
  ...
  const { data } = await supabase
    ...
    .select("id, request_number, status, payment_amount, payment_due_date, vendor:vendors(name)")
  ...
  const grandTotal = rows.reduce((s, r) => s + Number(r.payment_amount), 0);

  return (
    <div>
      <h1 className="text-2xl font-semibold text-zinc-900 dark:text-zinc-50">Cash-flow due</h1>
      <p className="mt-1 text-sm text-zinc-500">
        Approved requests not yet paid, grouped by when they&apos;re due.
      </p>
      ...
      <div className="mt-6 space-y-6">
        {grouped.map((g) => {
          ...
          const bTotal = g.rows.reduce((s, r) => s + Number(r.payment_amount), 0);
