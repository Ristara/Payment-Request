...
export default async function SpendReportPage({
  ...
  const { from: rawFrom, to: rawTo, saved } = await searchParams;
  const supabase = await createClient();

  // A saved layout carries its own date range, so opening one from Favourites
  // rebuilds exactly what was saved rather than whatever range happens to be
  // in the URL. An explicit ?from/?to still wins — that is the user changing
  // it after opening.
  const { data: savedRow } = saved
    ? await supabase.from("saved_reports").select("id, name, config").eq("id", saved).maybeSingle()
    : { data: null };
  const savedReport = savedRow as
    | { id: string; name: string; config: Record<string, unknown> }
    | null;
  const cfg = (savedReport?.config ?? {}) as { from?: string; to?: string };
  const from = rawFrom ?? cfg.from;
  const to = rawTo ?? cfg.to;
    ...
    <div>
      <div>
        <h1 className="text-2xl font-semibold text-zinc-900 dark:text-zinc-50">
          {savedReport?.name ?? "Create report"}
        </h1>
